...
bool HArray::insertOrGet(uint32* key,
						 uint32 keyLen,
						 uint32** pValue)
{
	try
	{
		...
		if (!contentOffset)
		{
			...
			if (tailReleasedContentOffsets[keyLen])
			{
				...
				*pValue = &pContentPage->pContent[contentIndex];
	...
	NEXT_KEY_PART:
			...
			if (contentIndex < maxSafeShort) //content in one page
			{
				for (; keyOffset < originKeyLen; contentOffset++, contentIndex++, keyOffset++)
				{
					...
					if (keyOffset == keyLen)
					{
						...
						*pValue = &pVarCell->ValueContCellValue;
		...
		if (contentCellType == VAR_TYPE) //VAR =====================================================================
		{
			...
			else
			{
				...
				*pValue = &varCell.ValueContCellValue;
