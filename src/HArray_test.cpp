...
uint32_t HArray::getFullContentLen(uint32_t contentOffset)
{
	uint8_t* pSourceStartContentCellType = &pContentPages[contentOffset >> 16]->pType[contentOffset & 0xFFFF];
	uint32_t* pSourceStartContentCellValue = &pContentPages[contentOffset >> 16]->pContent[contentOffset & 0xFFFF];
...
bool HArray::testVarConsistency()
{
	...
	if (ContentPagesCount > 0)
	{
		uint32_t lastPage = ContentPagesCount - 1;
...
bool HArray::testFillContentPages()
{
	...
	for (uint32_t cell = 0; cell < HeaderSize; cell++)
	{
		...
		if (contentOffset)
		{
			uint32_t len = getFullContentLen(contentOffset);

			for (uint32_t j = contentOffset; j < contentOffset + len; j++)
			{
				control[j]++;
			}
		}
		...
		for (uint32_t page = 0; page < BranchPagesCount; page++)
		{
			...
			for (uint32_t cell = 0; cell < countCells; cell++)
			{
				...
				for (uint32_t i = 0; i < BRANCH_ENGINE_SIZE; i++)
				{
					if (branchCell.Offsets[i]) //not empty
					{
						uint32_t len = getFullContentLen(branchCell.Offsets[i]);

						for (uint32_t j = branchCell.Offsets[i]; j < branchCell.Offsets[i] + len; j++)
						{
							control[j]++;
						}
					}
	...
	if (BlockPagesCount > 0)
	{
		...
		for (uint32_t page = 0; page < BlockPagesCount; page++)
		{
			...
			for (uint32_t cell = 0; cell < countCells; cell++)
			{
				...
				if (blockCell.Type == CURRENT_VALUE_TYPE)
				{
					uint32_t len = getFullContentLen(blockCell.Offset);

					for (uint32_t j = blockCell.Offset; j < blockCell.Offset + len; j++)
					{
						control[j]++;
					}
				}
	...
	if (VarPagesCount > 0)
	{
		...
		for (uint32_t page = 0; page < VarPagesCount; page++)
		{
			...
			for (uint32_t cell = 0; cell < countCells; cell++)
			{
				...
				if (varCell.ContCellType == CONTINUE_VAR_TYPE)
				{
					uint32_t len = getFullContentLen(varCell.ContCellValue);

					for (uint32_t j = varCell.ContCellValue; j < varCell.ContCellValue + len; j++)
					{
						control[j]++;
					}
				}
	...
	for (uint32_t i = 1; i < lastContentOffset; i++)
	{
		if (control[i] != 1)
		{
			delete[] control;
...
bool HArray::testFillBranchPages()
{
	...
	uint32_t currTailReleasedBranchOffset = tailReleasedBranchOffset;
	uint32_t currCountReleasedBranchCells = countReleasedBranchCells;
	...
	//check state
	for (uint32_t i = 0; i < currCountReleasedBranchCells; i++)
	{
	...
	for (uint32_t i = 0; i < lastBranchOffset; i++)
	{
		if (control[i] != 1)
		{
			delete[] control;
...
bool HArray::testFillBlockPages()
{
	...
	uint32_t currTailReleasedBlockOffset = tailReleasedBlockOffset;
	uint32_t currCountReleasedBlockCells = countReleasedBlockCells;
	...
	//check state
	for (uint32_t i = 0; i < currCountReleasedBlockCells; i += BLOCK_ENGINE_SIZE)
	{
	...
	for (uint32_t i = 0; i < lastBlockOffset / BLOCK_ENGINE_SIZE; i++)
	{
		if (control[i] != 1)
		{
			delete[] control;
...
bool HArray::testFillVarPages()
{
	...
	uint32_t currTailReleasedVarOffset = tailReleasedVarOffset;
	uint32_t currCountReleasedVarCells = countReleasedVarCells;
	...
	//check state
	for (uint32_t i = 0; i < currCountReleasedVarCells; i++)
	{
	...
	for (uint32_t i = 0; i < lastVarOffset; i++)
	{
		if (control[i] != 1)
		{
			delete[] control;
